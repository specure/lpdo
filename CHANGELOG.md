...
- **Windows: the pre-rebuild safety snapshot works** — it always failed with
  ...
  so the crash guard for index rebuilds and large imports was silently absent
  on Windows while printing that line on every heavy job. A byte copy of the
  live database is impossible there (DuckDB holds an exclusive lock on the
  file), so the snapshot is now taken *through* the open connection when the
  copy fails. Costs about two minutes on a 12 GB database and only affects
  rare, user-initiated heavy jobs — first-run setup skips snapshots entirely,
  so first-run timings are unchanged. (#246)
