...
Function PageReinstall
  ...
  Pop $R0
  ; ── LPDO (#244/#67): seamless upgrades ────────────────────────────────────
  ; A strictly NEWER version installs over the existing one exactly like the
  ; official updater's /UPDATE flow: skip this page and do NOT run the old
  ; uninstaller — so an upgrade never asks about uninstalling the previous
  ; version or about application data (whose checkbox, tied to our
  ; POSTUNINSTALL hook, would delete the ProgramData database). Same-version
  ; repair and downgrades keep the interactive page; WiX-era installs keep the
  ; migration path below.
  ${If} $R0 = 1
  ${AndIf} $WixMode <> 1
    StrCpy $UpdateMode 1
    Abort ; skip the reinstall page entirely
  ${EndIf}
  ; ─────────────────────────────────────────────────────────────────────────
...
; 4.5. Components page (#67): the client always installs (hidden required
; sections below); the background database server (WinSW service) is the one
; optional, default-on component. Hidden during passive installs and seamless
; upgrades (the previous choice keeps applying — the service hook re-registers
; only what is selected, and upgrades leave the selection at its default).
Function SkipComponentsPageIfUpdate
  ${If} $PassiveMode = 1
  ${OrIf} $UpdateMode = 1
    Abort
  ${EndIf}
FunctionEnd
!define MUI_PAGE_CUSTOMFUNCTION_PRE SkipComponentsPageIfUpdate
...
; Optional Windows-service component (#67). The body is empty on purpose: the
